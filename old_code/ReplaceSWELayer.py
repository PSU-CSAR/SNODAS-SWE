...
# ********************************************************************
# NOTE: Admin username and password has been removed and replaced with
# constants that are not set. In other words, this code will not run
# without setting the values of these constants:
#
ADMIN_NAME =
ADMIN_PASS =
#
        ...
        if remove_file(targetFilepathname) == 1:
            if copy_file(sourceFilepathname, targetFilepathname) == 1:
                if update_sourcedatadefinition(SNODAS_SourceDataFile, sourceFilename) == 1:
                    ...
                    stopStartServices(GIS_Server, Server_Port, ADMIN_NAME, ADMIN_PASS, "Start", SNODAS_ServiceName)
    else:
